...
# gang_filter = f1.selectbox("Gang Box", sorted(df["gang_box"].unique()))
# df1 = df[df["gang_box"] == gang_filter]

# if df1.empty:
#     st.warning("No data available for selected Gang Box.")
#     st.stop()

# switch_filter = f2.selectbox("Switches", sorted(df1["switches"].unique()))
# df2 = df1[df1["switches"] == switch_filter]

# fan_filter = f3.selectbox("Fan Modules", sorted(df2["fan_modules"].unique()))
# df3 = df2[df2["fan_modules"] == fan_filter]

# socket_filter = f4.selectbox("Sockets", sorted(df3["sockets"].unique()))
# df4 = df3[df3["sockets"] == socket_filter]

# dimmer_filter = f5.selectbox("Dimmers", sorted(df4["dimmers"].unique()))
...
#         ["-- Select --"] + filtered_df["product_name"].tolist()
#     )
# else:
...
# st.write(f"**Unit Price:**  ₹ {int(unit_price)}")
...
#         "placement": st.session_state.get("placement_input", ""),
#         "qty": qty,
#         "unit": float(final_row.get("mrp", 0)),
...
#         "Placement", "Qty", "Unit_price", "Total"
#     ]]

#     # ✅ Create wrapped styles INSIDE function
#     from reportlab.lib.styles import ParagraphStyle
#     from reportlab.lib.enums import TA_CENTER, TA_RIGHT
...
#     colWidths = [
#         1.2*cm,                         # S.No
#         total_available_width*0.22,     # Product
#         total_available_width*0.26,     # Description
#         total_available_width*0.18,     # Placement
#         1.2*cm,                         # Qty
#         total_available_width*0.10,     # Unit Price (slightly increased)
#         total_available_width*0.12      # Total
#     ]

#     # -------------------------------------------------
#     # MAIN TABLE (Items Only)
#     # -------------------------------------------------
#     table_data = [[
#         "S.No", "Product", "Description",
#         "Placement", "Qty", "Unit_price", "Total"
...
#     # -------------------------------------------------
#     # MAIN TABLE (Items + Totals Combined)
#     # -------------------------------------------------
#     table_data = [[
#         "S.No", "Product", "Description",
...
#     # Add totals rows inside same table
#     def total_row(label, value, bold=False, is_grand=False):
#         if is_grand:
#             style = grand_total_style
#         elif bold:
#             style = semi_bold_style
#         else:
#             style = right_style
...
#     total_row("Subtotal", subtotal, bold=True)
#     total_row("Discount", discount_amt)

#     if use_install:
#         total_row("Installation", installation)

#     if use_gst:
#         total_row("CGST", gst/2)
#         total_row("SGST", gst/2)

#     total_row("Grand Total", grand_total, is_grand=True)
...
#     # -------------------------------------------------
#     # TABLE STYLING WITH COLUMN SEPARATORS
#     # -------------------------------------------------
#     style_commands = [
...
#         # FULL GRID (column separators included)
...
#     # Zebra striping only for item rows
...
#     grand_total_index = len(table_data) - 1
#     style_commands.append(
#         ("BACKGROUND",(0,grand_total_index),(-1,grand_total_index),
#         colors.HexColor("#EFEFEF"))
#     )
#     style_commands.append(
#         ("LINEABOVE",(0,grand_total_index),(-1,grand_total_index),
#         1.2, colors.black)
#     )
...
#     return fname
##above pdf generation block is proper and working only BOQ excel part is added in below code so above part is replaced
...
#     # -------------------------------------------------
#     # WIDTH FIX (Installation wrapping fix)
#     # -------------------------------------------------
#     total_available_width = A4[0] - doc.leftMargin - doc.rightMargin
...
#         1.2*cm,
#         total_available_width*0.11,   # Increased slightly
#         total_available_width*0.13    # Increased slightly
#     ]
...
#     # Totals rows (NO extra blank row)
...
#     # -------------------------------------------------
#     # TABLE STYLE (with accounting double underline)
#     # -------------------------------------------------
#     style_commands = [
...
#     # Zebra striping only on product rows
#     for i in range(1, item_row_count + 1):
#         if i % 2 == 0:
#             style_commands.append(
#                 ("BACKGROUND",(0,i),(-1,i),colors.HexColor("#F7F7F7"))
...
#     )
#     style_commands.append(
#         ("LINEABOVE",(0,grand_index-0),(6,grand_index),0.6,colors.black)
#     )
...
#     with pd.ExcelWriter(boq_path) as writer:
#         boq_df.to_excel(writer, index=False, startrow=4)
...
#         worksheet.write("A1", "Kognitive IoT Solutions", header_format)
#         worksheet.write("A3", "BOQ", sub_format)

#     return fname



#THIS HAS FEW ADDITIONS, 
#✅ Excel not saving properly

#✅ Customer details not reflecting in PDF

#✅ Proper zebra striping for product rows
...
def generate_pdf(doc_number):
    ...
    # =====================================================
        # =====================================================
    # BOQ EXCEL GENERATION (NO PRICE) - HARD FIX
    # =====================================================
        # =====================================================
